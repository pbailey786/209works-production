...
function testBusinessDashboard() {
  ...
  try {
    ...
    const response = execSync('curl -s -o /dev/null -w "%{http_code}" http://localhost:3001/demo/business-dashboard', { encoding: 'utf8' });
    ...
    const pageContent = execSync('curl -s http://localhost:3001/demo/business-dashboard', { encoding: 'utf8' });
    ...
    console.log('\n📊 Test 3: Analytics Tracking Demo');
    const analyticsResponse = execSync('curl -s -o /dev/null -w "%{http_code}" http://localhost:3001/demo/analytics-tracking', { encoding: 'utf8' });
    
    if (analyticsResponse.trim() === '200') {
      console.log('✅ Analytics tracking demo accessible (HTTP 200)');
    } else {
      console.log(`⚠️  Analytics tracking demo issue (HTTP ${analyticsResponse.trim()})`);
    }

    // Test 4: PostHog Analytics Demo
    console.log('\n🔍 Test 4: PostHog Analytics Demo');
    const posthogResponse = execSync('curl -s -o /dev/null -w "%{http_code}" http://localhost:3001/demo/posthog-analytics', { encoding: 'utf8' });
    
    if (posthogResponse.trim() === '200') {
      console.log('✅ PostHog analytics demo accessible (HTTP 200)');
    } else {
      console.log(`⚠️  PostHog analytics demo issue (HTTP ${posthogResponse.trim()})`);
    }

    // Test 5: Environment Variables Check
    console.log('\n🔧 Test 5: Environment Configuration');
    try {
      const envContent = execSync('type .env 2>nul || echo "No .env file"', { encoding: 'utf8' });
    ...
    console.log('• http://localhost:3001/demo/business-dashboard - Main business metrics dashboard');
    console.log('• http://localhost:3001/demo/analytics-tracking - Job board analytics tracking');
    console.log('• http://localhost:3001/demo/posthog-analytics - PostHog integration demo');
    
    console.log('\n💡 Features Implemented:');
    console.log('• Comprehensive KPI tracking (Users, Jobs, Applications, Revenue)');
