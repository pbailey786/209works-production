...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[CRON] Job rankings update completed in ${processingTime}ms`, results);
...
async function updateJobPopularityScores(): Promise<void> {
  ...
  for (const job of jobs) {
    ...
    const daysOld = Math.floor((Date.now() - job.createdAt.getTime()) / (1000 * 60 * 60 * 24));
    if (daysOld > 14) {
...
async function processSearchAnalytics(): Promise<number> {
  ...
  try {
    ...
    console.log('[CRON] Top trending search terms:', trendingTerms.map(([term, metrics]) => ({
      term,
