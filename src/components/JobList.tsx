...
function jobListReducer(state: JobListState, action: JobListAction): JobListState {
  switch (action.type) {
    ...
    case 'SET_FETCH_STATE':
      return { ...state, fetchState: { ...state.fetchState, ...action.payload } };
    case 'SET_FETCHED':
...
export default function JobBoard() {
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      ...
      dispatch({ type: 'SET_DEBOUNCED_LOCATION', payload: state.searchLocation });
    }, 500); // 500ms debounce
  ...
  const fetchJobs = async (isRetry: boolean = false) => {
    ...
    dispatch({ type: 'SET_FETCH_STATE', payload: {
      isLoading: true,
      hasError: false,
      errorMessage: null,
    ...
    try {
      ...
      dispatch({ type: 'SET_TOTAL_PAGES', payload: Math.max(1, Math.ceil((data.total || 0) / pageSize)) });
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-screen">
      {/* Left: Search + Jobs */}
      <div className="lg:w-[35%] bg-gradient-to-b from-purple-50 to-white p-6 overflow-y-auto">
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row items-center gap-4 w-full max-w-3xl mb-6">
          ...
          />
          <button
            className="px-5 py-3 bg-purple-700 text-white rounded-lg hover:bg-purple-800 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center min-w-[100px]"
            type="submit"
        ...
        {state.fetchState.isLoading && (
          <div className="flex flex-col items-center justify-center py-12">
            <LoadingSpinner size="lg" variant="spinner" color="primary" message="Loading jobs..." />
          </div>
        )}
        ...
        {/* Empty State */}
        {!state.fetchState.isLoading && !state.fetchState.hasError && state.fetched && state.jobs.length === 0 && (
          <div className="flex flex-col items-center justify-center mt-16">
            <svg className="w-24 h-24 text-purple-200 mb-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 48 48">
            ...
            <div className="text-lg text-gray-500 text-center">
              ...
              <p className="text-sm mt-1">Try adjusting your search criteria or check back later</p>
            </div>
          </div>
        )}
        ...
        {!state.fetchState.isLoading && !state.fetchState.hasError && (
          <div role="list" aria-label={`${state.jobs.length} job${state.jobs.length !== 1 ? 's' : ''} found`}>
              ...
              <div key={job.id} role="listitem">
                ...
                <div className="mb-4">
                  <JobCard
                    ...
                    onClick={() => dispatch({ type: 'SET_SELECTED_JOB', payload: job })}
        ...
        {!state.fetchState.isLoading && state.jobs.length > 0 && (
          <div className="flex justify-center mt-6 mb-4">
            <nav className="flex items-center gap-2" aria-label="Job search pagination">
              <button
              ...
              <button
                ...
                disabled={state.page === state.totalPages || state.fetchState.isLoading}
                className="px-3 py-1 rounded border disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors"
      ...
      {/* Center: Job Details */}
      <div className="lg:w-[45%] bg-white p-6 overflow-y-auto flex items-center justify-center" role="main" aria-label="Job details">
        {state.selectedJob ? (
          <div className="max-w-2xl w-full">
            <h2 id="job-details-title" className="text-2xl font-bold mb-2">{state.selectedJob.title}</h2>
            <div className="text-gray-600 mb-1" aria-label="Company name">{state.selectedJob.company}</div>
            <div className="text-sm text-gray-400 mb-4" aria-label="Posted date">
            ...
            <div className="flex gap-3 mt-6" role="group" aria-label="Job actions">
              <button
                onClick={() => state.selectedJob && handleViewDetails(state.selectedJob)}
              ...
              <a
                ...
                className="inline-block px-6 py-3 rounded-lg bg-white text-purple-700 border border-purple-200 font-semibold hover:bg-purple-50 transition"
                target="_blank"
      ...
      {/* Right: Sidebar Ads */}
      <div className="lg:w-[20%] bg-gray-50 p-4 overflow-y-auto border-l border-gray-200">
        <div className="space-y-6">
          {/* Sidebar Ads */}
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-3">Featured Businesses</h3>
            <AdDisplay 
              placement="sidebar" 
              maxAds={3}
              userLocation={state.location}
          ...
          {/* Native Ads */}
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-3">Recommended</h3>
            <AdDisplay 
              placement="native" 
              maxAds={2}
              userLocation={state.location}
      ...
      <EnhancedJobModal
        ...
        saved={state.selectedJob ? state.savedJobs.includes(state.selectedJob.id) : false}
