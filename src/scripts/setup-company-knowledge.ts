...
async function setupCompanyKnowledge() {
  console.log(
    '🏢 Setting up Company Knowledge Base for Phase 2 (with sample data)\n'
  );
  ...
  try {
    ...
    for (const companyData of SAMPLE_COMPANIES) {
      try {
        ...
        if (!company) {
          const companyId =
            await CompanyKnowledgeService.createCompanyFromJobData(
              companyData.name
          ...
          console.log(
            `ℹ️ Company ${companyData.name} already exists. Skipping creation, will update/seed knowledge.`
          );
          createdCompanies.push(company);
        }

        // Ensure existing jobs are linked (if any)
        const linkedCount = await CompanyKnowledgeService.linkJobsToCompany(
          company.name,
          company.id
        );
        if (linkedCount > 0) {
          console.log(
            `🔗 Linked ${linkedCount} existing jobs to ${company.name}`
          );
        }
        ...
        await CompanyKnowledgeService.seedDefaultKnowledge(
          company.id,
          company.name
        );
    ...
    console.log(`\nSummary:`);
    console.log(
      `- Processed ${createdCompanies.length} sample company records`
    );
...
async function addSampleKnowledgeData(companies: any[]) {
  for (const company of companies) {
    ...
    if (company.name === 'AlphaCorp') {
      // Add specific extra knowledge for one company
      await CompanyKnowledgeService.addCompanyKnowledge({
        companyId: company.id,
