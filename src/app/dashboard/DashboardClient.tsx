...
export default function DashboardClient({
  ...
  const formatFilters = (filters: any) => {
    ...
    if (filters.salaryMin) filterParts.push(`Min Salary: $${filters.salaryMin.toLocaleString()}`);
            ...
            <div className="space-y-4">
              {recentSavedJobs.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="mt-1 text-sm text-gray-500">Start saving jobs you're interested in.</p>
                  <div className="mt-6">
                    <Link
                      href="/jobs"
                      className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-[#2d4a3e] hover:bg-[#1d3a2e]"
                    >
                      Browse Jobs
                    </Link>
                  </div>
                ...
                <>
                    ...
                    <div key={job.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right text-sm text-gray-500">
                          <p>Saved {formatDistanceToNow(new Date(job.savedAt), { addSuffix: true })}</p>
                          <p className="mt-1">Posted {formatDistanceToNow(new Date(job.postedAt), { addSuffix: true })}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                  <div className="text-center pt-4">
                    <Link
                      href="/profile/saved"
                      className="text-[#2d4a3e] hover:text-[#1d3a2e] text-sm font-medium"
                    >
                      View all saved jobs →
                    </Link>
                  </div>
                  ...
                  {recentSearches.map((search) => (
                    <div key={search.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right text-sm text-gray-500">
                          <p>{formatDistanceToNow(new Date(search.createdAt), { addSuffix: true })}</p>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="space-y-4">
              {recentAlerts.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="mt-1 text-sm text-gray-500">Set up alerts to get notified about new jobs.</p>
                  <div className="mt-6">
                    <Link
                      href="/alerts"
                      className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-[#2d4a3e] hover:bg-[#1d3a2e]"
                  ...
                  {recentAlerts.map((alert) => (
                    <div key={alert.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right text-sm text-gray-500">
                          <p>Created {formatDistanceToNow(new Date(alert.createdAt), { addSuffix: true })}</p>
                        </div>
                      </div>
                    </div>
                  ))}
