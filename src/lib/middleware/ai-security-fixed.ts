...
/**
 * AI Security Middleware - Fixed version with Clerk integration
...
/**
 * Main AI security middleware - Fixed with Clerk integration
 */
export function withAISecurity(
  ...
  return async (req: NextRequest): Promise<NextResponse> => {
    ...
    try {
      // 1. Authentication check with Clerk
      let user = null;
      let isAuthenticated = false;

      // Check if Clerk auth is enabled
      const clerkEnabled = process.env.NEXT_PUBLIC_ENABLE_CLERK_AUTH === 'true';
      
      if (config.requireAuthentication || clerkEnabled) {
        try {
          // Get Clerk user
          const clerkUser = await currentUser();
          
          if (config.requireAuthentication && !clerkUser) {
          ...
          if (clerkUser) {
            // Try to find user in database by Clerk ID first, then by email
            ...
            // If user not found in DB but exists in Clerk, create a basic user record
              ...
              try {
                user = await prisma.user.create({
                  data: {
                    ...
                    passwordHash: 'clerk-auth', // Placeholder since Clerk handles auth
                  },
                  select: { id: true, email: true, role: true },
                });
              } catch (createError) {
                // User might already exist with different ID, ignore
                console.log('Could not create user from Clerk data:', createError);
              }
            }
            ...
            if (user && config.allowedRoles && !config.allowedRoles.includes(user.role)) {
              await logAIRequest(
                user.id,
                '',
                ipAddress,
          ...
          console.error('Clerk auth error in AI security:', authError);
          // Continue without auth if it's not required
