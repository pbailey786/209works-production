...
async function checkExternalAPI(name: string, url: string): Promise<{
  name: string;
  status: 'healthy' | 'unhealthy';
  responseTime: number;
  error?: string;
...
function checkMemory(): { status: 'healthy' | 'unhealthy'; usage: number; limit: number } {
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      if (process.env.GOOGLE_CLIENT_ID) {
        externalServices.push(
          checkExternalAPI('Google OAuth', 'https://accounts.google.com/.well-known/openid_configuration')
        );
      }
    }
    ...
    if (externalChecks.some(check => check.status === 'unhealthy')) {
      externalApiStatus = externalChecks.every(check => check.status === 'unhealthy') ? 'unhealthy' : 'degraded';
    }

    // Determine overall system status
    let overallStatus: 'healthy' | 'unhealthy' | 'degraded' = 'healthy';
    
    if (databaseCheck.status === 'unhealthy' || redisCheck.status === 'unhealthy' || memoryCheck.status === 'unhealthy') {
...
export async function HEAD(): Promise<NextResponse> {
  try {
    ...
    const quickCheck = await Promise.race([
      checkDatabase(),
      new Promise<{ status: 'unhealthy'; responseTime: number; error: string }>((_, reject) => 
        setTimeout(() => reject({ status: 'unhealthy', responseTime: 1000, error: 'Timeout' }), 1000)
      )
    ]);
