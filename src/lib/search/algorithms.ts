...
export class TextProcessor {
    ...
    try {
      ...
      const words = normalized.split(' ').filter(word => {
        ...
        return (
          ...
          !SEARCH_CONFIG.fullText.stopWords.includes(word as typeof SEARCH_CONFIG.fullText.stopWords[number])
        );
      });
  ...
  static createSnippet(
    ...
    // Input validation
    if (!text || typeof text !== 'string') return '';
    if (!query || typeof query !== 'string') return text.substring(0, maxLength);
...
export class RelevanceScorer {
  ...
  static scoreJob(job: any, query: string, filters: EnhancedSearchFilters = {}): number {
    // Input validation
...
export class GeolocationUtils {
  ...
  static calculateDistance(
    ...
    if (!InputValidator.isValidCoordinate(lat1, lng1) || 
  ...
  static getBoundingBox(
    ...
    if (!InputValidator.isValidNumber(radiusMiles) || radiusMiles <= 0 || radiusMiles > 1000) {
    ...
    try {
      ...
      if (Math.abs(cosLat) < 0.0001) {
        console.warn('Coordinate too close to poles, using fallback calculation');
      ...
      if (!InputValidator.isValidNumber(latDelta) || !InputValidator.isValidNumber(lngDelta)) {
      ...
      // Validate result
      if (!InputValidator.isValidCoordinate(result.minLat, result.minLng) ||
  ...
  static async geocodeLocation(location: string): Promise<{lat: number; lng: number} | null> {
...
export class FacetedSearch {
  ...
  static generateFacets(items: any[], searchField: string = 'jobs'): Record<string, any> {
    // Input validation
