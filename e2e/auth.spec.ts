...
test.describe('Authentication', () => {
  ...
  test('should show validation errors for invalid sign in', async ({ page }) => {
    ...
    if (await emailInput.isVisible() && await passwordInput.isVisible() && await signInButton.isVisible()) {
      ...
      const errorMessage = page.locator('.error, .alert-error, [role="alert"], :has-text("Invalid")')
  ...
  test('should navigate between sign in and sign up pages', async ({ page }) => {
    ...
    if (await signUpLink.isVisible()) {
      ...
      const signInLink = page.locator('a[href*="signin"], a:has-text("Sign In"), a:has-text("Login")')
  ...
  test('should handle password reset flow', async ({ page }) => {
    ...
    const forgotPasswordLink = page.locator('a:has-text("Forgot"), a:has-text("Reset"), a[href*="reset"]')
    ...
    if (await forgotPasswordLink.isVisible()) {
      ...
      const emailInput = page.locator('input[type="email"], input[name="email"]')
      const resetButton = page.locator('button[type="submit"], button:has-text("Reset")')
      ...
      if (await emailInput.isVisible() && await resetButton.isVisible()) {
        ...
        const successMessage = page.locator('.success, .alert-success, :has-text("sent"), :has-text("check")')
