...
// GET /api/dashboard/profile-completion - Get profile completion status
export async function GET(req: NextRequest) {
  try {
    ...
    const resume = !!(profile?.resumeData && 
      (profile.resumeData as any)?.workHistory?.length > 0);
    ...
    const availability = !!(
      ...
      profile?.availabilityShifts &&
      profile.availabilityShifts.length > 0
    );

    // Calculate overall completion percentage
    const sections = [basicInfo, resume, skills, preferences, availability];
