...
// Note: PUT and DELETE are not exported from this route`,
  ...
  apiCallSignature: {
    ...
    fix: (match, method) =>
      `const response = await ${method}(req, { params: {} });`,
  },
...
const utilFiles = ['src/utils/modal-accessibility.ts'];

function fixTestingLibraryImports(content) {
  // Fix @testing-library/react imports
  if (
    content.includes('screen, fireEvent, waitFor') &&
...
function fixApiCallSignatures(content) {
  ...
  content = content.replace(
    ...
    (match, method, id) =>
      `await ${method}(req, { params: Promise.resolve({ id: ${id} }) });`
  );
...
function fixMockTypes(content) {
  ...
  content = content.replace(
    ...
    (match, model, method, value) =>
      `prismaMock.${model}.${method}.mockResolvedValue(${value} as any);`
  );
...
function fixArrayMethods(content) {
  ...
  content = content.replace(/\.path\.join\(/g, '.join(');
  return content;
}
