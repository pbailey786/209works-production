...
import { CommunityForumsService } from '@/lib/social/community-forums';
import { z } from 'zod';
...
const createPostSchema = z.object({
  ...
  type: z.enum(['discussion', 'question', 'job_share', 'advice', 'announcement']),
...
// POST /api/social/forums - Handle forum actions
  ...
  try {
    ...
    switch (action) {
      case 'create_post':
        ...
        const post = await CommunityForumsService.createPost(
          user.id,
      ...
      case 'create_comment':
        const commentData = createCommentSchema.parse(data);
      ...
      case 'vote':
        ...
        const voteResult = await CommunityForumsService.vote(
          user.id,
          voteData.targetId,
          voteData.targetType,
          voteData.voteType
    ...
    console.error('Error in forums API:', error);
...
// GET /api/social/forums - Get forum data
export async function GET(req: NextRequest) {
  try {
    const url = new URL(req.url);
    const action = url.searchParams.get('action') || 'get_categories';

    switch (action) {
      case 'get_categories':
        const categories = await CommunityForumsService.getForumCategories();
      ...
      case 'get_posts':
        ...
        const posts = await CommunityForumsService.getCategoryPosts(
          categorySlug,
          {
            page,
            limit,
            sortBy,
            filterBy: { type, tags, timeframe },
            userId: user?.id,
          }
        );
      ...
      case 'get_post':
        ...
        const postDetails = await CommunityForumsService.getPostDetails(
          postId,
          currentUser?.id
        );
      ...
      case 'get_user_reputation':
        ...
        const reputation = await CommunityForumsService.getUserReputation(targetUserId);
