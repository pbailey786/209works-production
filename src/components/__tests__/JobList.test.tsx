  ...
  return function MockEnhancedJobModal({ 
    ...
    return (
      <div data-testid="enhanced-job-modal">
        ...
        <button onClick={onApply} data-testid="modal-apply">Apply</button>
        <button onClick={onShare} data-testid="modal-share">Share</button>
      </div>
    )
  }
})
...
describe('JobBoard Component', () => {
  ...
  describe('Rendering', () => {
    it('renders the search form correctly', () => {
      ...
      expect(screen.getByPlaceholderText('Job title, keyword')).toBeInTheDocument()
      expect(screen.getByPlaceholderText('Location')).toBeInTheDocument()
      expect(screen.getByRole('button', { name: /search for jobs/i })).toBeInTheDocument()
    })
  ...
  describe('Search Functionality', () => {
    it('handles form submission and triggers search', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({ jobs: mockJobs, total: 2 }))
  ...
  describe('Job Loading and Display', () => {
    ...
    it('displays jobs after successful fetch', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('job-card-Frontend Developer')).toBeInTheDocument()
        expect(screen.getByTestId('job-card-Backend Engineer')).toBeInTheDocument()
      })
    })
    ...
    it('displays no jobs found message when search returns empty results', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({ jobs: [], total: 0 }))
  ...
  describe('Job Selection and Details', () => {
    ...
    const setupJobsTest = async () => {
      mockFetch.mockResolvedValue(createMockResponse({ jobs: mockJobs, total: 2 }))
    ...
    it('selects job and displays details when job card is clicked', async () => {
      ...
      await waitFor(() => {
        ...
        const jobDetailsHeading = screen.getByRole('heading', { level: 2, name: 'Frontend Developer' })
        ...
        expect(jobDetailsContainer).toHaveTextContent('We are looking for a skilled frontend developer...')
      })
    })
  ...
  describe('Enhanced Job Modal', () => {
    ...
    it('opens modal when View Details button is clicked', async () => {
      ...
      const viewDetailsButton = screen.getByTestId('view-details-Frontend Developer')
    ...
    it('closes modal when close button is clicked', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('enhanced-job-modal')).not.toBeInTheDocument()
      })
    })
  ...
  describe('Pagination', () => {
    it('displays pagination controls when there are multiple pages', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({ jobs: mockJobs, total: 50 })) // More than 20 jobs
    ...
    it('handles next page navigation', async () => {
      mockFetch
        .mockResolvedValueOnce(createMockResponse({ jobs: mockJobs, total: 50 }))
        .mockResolvedValueOnce(createMockResponse({ jobs: [mockJobs[0]], total: 50 }))
    ...
    it('disables Previous button on first page', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({ jobs: mockJobs, total: 50 }))
  ...
  describe('Accessibility', () => {
    ...
    it('has proper screen reader labels', () => {
      ...
      expect(screen.getByText('Job title or keyword', { selector: '.sr-only' })).toBeInTheDocument()
      expect(screen.getByText('Location', { selector: '.sr-only' })).toBeInTheDocument()
    })
  })
}) 