...
function fixFile(filePath) {
  try {
    ...
    if (content.includes('prisma.') && !content.includes("import { prisma }")) {
      ...
      lines.splice(insertIndex, 0, "import { prisma } from '@/lib/database/prisma';");
      content = lines.join('\n');
      modified = true;
    }
    ...
    if (filePath.includes('/actions/') && content.includes('userId') && !content.includes('const { userId }')) {
      content = content.replace(
        /^(\s*'use server';?\s*\n)/m,
    ...
    if (filePath.includes('/api/') && content.includes('user?.') && !content.includes('const user =')) {
