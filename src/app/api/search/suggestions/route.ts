...
const suggestionsQuerySchema = z.object({
  ...
  category: z.enum(['all', 'jobs', 'locations', 'companies', 'skills']).default('all'),
...
export const GET = withAPIMiddleware(
  async (req, context) => {
    ...
    switch (validType) {
      case 'trending':
        suggestions = await generateTrendingSuggestions(validCategory, validLimit, performance);
        break;
      case 'popular':
        suggestions = await generatePopularSuggestions(validCategory, validLimit, performance);
        break;
      case 'recent':
        suggestions = await generateRecentSuggestions(validCategory, validLimit, performance);
        break;
      default:
        suggestions = [];
    }
...
async function generateTrendingSuggestions(
  ...
  if (category === 'all' || category === 'skills') {
    ...
    const trendingSkills = [
      { query: 'artificial intelligence', type: 'skill', frequency: 89, trend: 'up' },
