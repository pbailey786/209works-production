...
function fixSpecificErrors(content, filePath) {
  ...
  if (
    ...
    content.includes("body: 'invalid json'")
  ) {
    content = content.replace(
...
function fixFile(filePath) {
  try {
    ...
    const { content: content1, hasChanges: changes1 } = fixNextJS15Params(
      content,
      filePath
    );
    content = content1;
    ...
    const { content: content2, hasChanges: changes2 } = fixSpecificErrors(
      content,
      filePath
    );
    content = content2;
