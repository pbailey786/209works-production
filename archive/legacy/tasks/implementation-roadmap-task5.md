...
interface ConversationContext {
  ...
  userId?: string;
  messages: Message[];
  intent: 'job_search' | 'company_info' | 'career_guidance' | 'application_help';
  context: {
    currentJobs?: Job[];
...
class CompanyKnowledgeRetriever {
  async getCompanyInfo(companyName: string, category?: string): Promise<CompanyInfo>
...
class IntentRouter {
  async classifyIntent(message: string, context: ConversationContext): Promise<Intent>
  async routeToHandler(intent: Intent, message: string, context: ConversationContext): Promise<Response>
}
```
