  ...
  async (req, context) => {
    ...
    return getCacheOrExecute(
      ...
      async () => {
        ...
        const recentMatches = Array.from({ length: Math.min(5, Math.floor(Math.random() * 10)) }, (_, i) => ({
          id: `job-${i + 1}`,
        ...
        const stats = {
          ...
          estimatedNextRun: calculateNextRun(alert.frequency, alert.lastTriggered),
        };
...
function calculateNextRun(frequency: string, lastSent: Date | null): string | null {
