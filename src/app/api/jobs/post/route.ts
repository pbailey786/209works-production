...
export const POST = withAPIMiddleware(
  async (req: NextRequest, context) => {
    try {
      ...
      if (existingJob) {
        return NextResponse.json(
          { error: 'A similar job posting already exists. Please wait 24 hours before posting the same job again.' },
          { status: 409 }
        );
      }

      // Generate embedding for job content
      const embeddingVector = await getEmbedding(`${validatedData.title} ${validatedData.description}`);
      ...
      const job = await prisma.job.create({
        data: {
          ...
          postedAt: validatedData.postedAt ? new Date(validatedData.postedAt) : new Date(),
          embedding,
