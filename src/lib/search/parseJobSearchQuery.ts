...
export function parseJobSearchQuery(query: string): ParsedJobSearchQuery {
  ...
  // Job type
  const jobTypeMatch = q.match(/\b(full[- ]?time|part[- ]?time|contract|internship|temporary|freelance)\b/i);
  if (jobTypeMatch) {
  ...
  const salaryMatch = q.match(/(?:\$|£|€)?\s?(\d{2,3}(?:,\d{3})?)(k|K)?\s*(\+|or more|per hour|per year)?/i);
  if (salaryMatch) {
  ...
  const keywords = q
    ...
    .trim()
    .split(' ')
    .filter(word => word.length > 2 && !['jobs', 'job', 'positions', 'openings'].includes(word.toLowerCase()));
