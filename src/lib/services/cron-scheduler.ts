...
export class CronSchedulerService {
   ...
   */
  private async processEmailAlerts(frequency: 'immediate' | 'daily'): Promise<void> {
    ...
    for (const alert of alerts) {
      try {
        ...
        const isUnsubscribed = await this.checkIfUserUnsubscribed(alert.user.email, 'job_alert');
        if (isUnsubscribed) {
          continue;
        }
  ...
  private async processWeeklyDigests(): Promise<void> {
    ...
    for (const digest of digests) {
      try {
        // Skip if user has unsubscribed
        const isUnsubscribed = await this.checkIfUserUnsubscribed(digest.user.email, 'weekly_digest');
        if (isUnsubscribed) {
          continue;
        }
  ...
  private async cleanupExpiredTokens(): Promise<void> {
    ...
    console.log(`[CRON] Cleanup completed: ${magicLinkResult.count} magic links, ${passwordResetResult.count} password resets, ${emailLogResult.count} email logs`);
  }
  ...
  private async updateJobRankings(): Promise<void> {
    ...
    console.log(`[CRON] Updated job rankings and marked ${expiredJobsResult.count} jobs as expired`);
  }
  ...
  private async performDatabaseMaintenance(): Promise<void> {
    try {
      ...
      console.log(`[CRON] Database maintenance: cleaned ${analyticsResult.count} old search analytics`);
    } catch (error) {
  ...
  private async findMatchingJobs(alert: any) {
    ...
    if (alert.location) {
      searchCriteria.location = { contains: alert.location, mode: 'insensitive' };
    }
  ...
  private async getJobsForDigest(digest: any) {
    ...
    return recentJobs.map(job => ({
      ...
      salary: job.salaryMin && job.salaryMax 
        ? (() => {
            try {
              const minSalary = typeof job.salaryMin === 'number' && isFinite(job.salaryMin) 
                ? job.salaryMin.toLocaleString() 
                : '0';
              const maxSalary = typeof job.salaryMax === 'number' && isFinite(job.salaryMax) 
