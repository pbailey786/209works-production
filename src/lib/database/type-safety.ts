...
const JobCreateBaseSchema = z.object({
  ...
  jobType: z.enum(['full_time', 'part_time', 'contract', 'internship', 'temporary', 'volunteer', 'other']),
...
export const JobCreateSchema = JobCreateBaseSchema.refine((data) => {
...
export const UserCreateSchema = z.object({
  ...
  preferredJobTypes: z.array(z.enum(['full_time', 'part_time', 'contract', 'internship', 'temporary', 'volunteer', 'other'])).max(7),
...
export const CompanyCreateSchema = z.object({
  ...
  contactPhone: z.string().regex(/^\+?[\d\s\-\(\)]+$/).max(20).optional(),
...
const AlertCreateBaseSchema = z.object({
  ...
  location: z.string().max(200).optional(),
  categories: z.array(z.string().max(50)).max(10),
  jobTypes: z.array(z.enum(['full_time', 'part_time', 'contract', 'internship', 'temporary', 'volunteer', 'other'])).max(7),
...
// Apply refine to create the full schema
export const AlertCreateSchema = AlertCreateBaseSchema.refine((data) => {
  if (data.salaryMin && data.salaryMax && data.salaryMin > data.salaryMax) {
    return false;
  }
  return true;
...
export const AlertUpdateSchema = AlertCreateBaseSchema.partial().omit({ userId: true });
...
export class TypeSafePrisma {
  ...
  async createJob(data: z.infer<typeof JobCreateSchema>) {
    ...
    if (validatedData.companyId) {
      ...
      if (!companyExists) {
        throw new Error(`Company with ID ${validatedData.companyId} does not exist`);
      }
    }
  ...
  async createCompany(data: z.infer<typeof CompanyCreateSchema>) {
    ...
    if (nameConflict) {
      throw new Error(`Company with name "${validatedData.name}" already exists`);
    }
    
    if (slugConflict) {
      throw new Error(`Company with slug "${validatedData.slug}" already exists`);
    }
  ...
  async updateCompany(id: string, data: z.infer<typeof CompanyUpdateSchema>) {
    ...
    if (validatedData.name && validatedData.name !== existingCompany.name) {
      ...
      if (nameConflict) {
        conflicts.push(`Company name "${validatedData.name}" is already in use`);
      }
    }
    ...
    if (validatedData.slug && validatedData.slug !== existingCompany.slug) {
      ...
      if (slugConflict) {
        conflicts.push(`Company slug "${validatedData.slug}" is already in use`);
      }
    }
  ...
  async createAlert(data: z.infer<typeof AlertCreateSchema>) {
    ...
    if (alertCount >= 20) {
      throw new Error('Maximum number of active alerts (20) reached for this user');
    }
  ...
  async updateUserMetadata(userId: string, metadata: Record<string, any>) {
    ...
    const MetadataSchema = z.record(z.any()).refine((data) => {
      try {
        JSON.stringify(data);
        return true;
      } catch {
  ...
  async createJobsBatch(jobs: z.infer<typeof JobCreateSchema>[]) {
    ...
    const validatedJobs = jobs.map((job, index) => {
      try {
        ...
        throw new Error(`Validation failed for job at index ${index}: ${error}`);
      }
    });
  ...
  async findJobsWithFilters(filters: {
    ...
    const FilterSchema = z.object({
      location: z.string().max(200).optional(),
      jobType: z.enum(['full_time', 'part_time', 'contract', 'internship', 'temporary', 'volunteer', 'other']).optional(),
      salaryMin: z.number().int().min(0).max(10000000).optional(),
      salaryMax: z.number().int().min(0).max(10000000).optional(),
...
export const TypeGuards = {
  ...
  isValidJobType: (value: any): value is string => {
    return typeof value === 'string' && 
           ['full_time', 'part_time', 'contract', 'internship', 'temporary', 'volunteer', 'other'].includes(value);
  },
  
  isValidUserRole: (value: any): value is string => {
    return typeof value === 'string' && ['admin', 'employer', 'jobseeker'].includes(value);
  },
