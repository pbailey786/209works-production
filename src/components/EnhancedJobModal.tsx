...
export default function EnhancedJobModal({
  ...
  const [jobDataState, setJobDataState] = useState<AsyncOperationState>(createAsyncOperationState());
  const [saveState, setSaveState] = useState<AsyncOperationState>(createAsyncOperationState());
  const [shareState, setShareState] = useState<AsyncOperationState>(createAsyncOperationState());
  const [applyState, setApplyState] = useState<AsyncOperationState>(createAsyncOperationState());

  // Generate enhanced job data when modal opens with validation
  useEffect(() => {
    if (isOpen && job) {
      const generateJobData = async () => {
        setJobDataState(startAsyncOperation(jobDataState, 'Loading job details...'));
  ...
  const handleSave = async () => {
    if (!isAuthenticated) {
      setSaveState(handleAsyncError(
        new Error('Please sign in to save jobs'), 
        saveState
      ));
      return;
    }
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <h1 id="job-modal-title" className="text-2xl font-bold mb-1">{enhancedJob.title}</h1>
                    <div id="job-modal-description" className="flex items-center text-purple-100 mb-2">
                ...
                <div className="flex items-center gap-6 text-sm">
                  <div className="flex items-center">
                    ...
                    <span aria-label={`${enhancedJob.viewsCount} people have viewed this job`}>
                      {enhancedJob.viewsCount} views
                    </span>
                  </div>
                  <div className="flex items-center">
                    <UserGroupIcon className="h-4 w-4 mr-1" aria-hidden="true" />
                    <span aria-label={`${enhancedJob.applicantsCount} people have applied to this job`}>
                      {enhancedJob.applicantsCount} applicants
                    </span>
                  </div>
                  <div className="flex items-center">
                    <CurrencyDollarIcon className="h-4 w-4 mr-1" aria-hidden="true" />
                    <span aria-label={`Salary range: ${safeFormatSalary(enhancedJob.salaryMin, enhancedJob.salaryMax)}`}>
                      {safeFormatSalary(enhancedJob.salaryMin, enhancedJob.salaryMax)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  } ${saveState.isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                  aria-label={saved ? `Job ${enhancedJob.title} is saved` : `Save job ${enhancedJob.title}`}
                >
                  {saveState.isLoading ? (
                ...
                <button
                  ...
                  className={`flex items-center px-4 py-2 rounded-lg text-sm font-medium bg-white text-purple-600 hover:bg-purple-50 transition ${
                    shareState.isLoading ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
          ...
          {(saveState.hasError || shareState.hasError || applyState.hasError) && (
                ...
                {activeTab === 'overview' && (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{enhancedJob.experienceLevel}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Job Type:</span>
                            <span className="font-medium">{enhancedJob.type}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Location:</span>
                            <span className="font-medium">{enhancedJob.location}</span>
                          </div>
                      ...
                      <div>
                        <h4 className="font-medium text-gray-900 mb-3">Skills Required</h4>
                    ...
                    {/* Benefits Preview */}
                    <div>
                      <h4 className="font-medium text-gray-900 mb-3">Benefits Highlights</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {enhancedJob.benefits.slice(0, 4).map((benefit, index) => (
                      ...
                      {enhancedJob.benefits.length > 4 && (
                        <button
                          ...
                          className="text-purple-600 text-sm font-medium mt-2 hover:text-purple-700"
                        >
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        {['required', 'preferred', 'nice-to-have'].map((importance) => {
                          const skillsOfType = enhancedJob.skills.filter(s => s.importance === importance);
                          if (skillsOfType.length === 0) return null;
                          
                          return (
                            <div key={importance}>
                              <h4 className={`font-medium mb-2 capitalize ${
                                importance === 'required' ? 'text-red-700' : 
                                importance === 'preferred' ? 'text-yellow-700' : 'text-green-700'
                              }`}>
                                {importance} Skills
                              </h4>
                ...
                {activeTab === 'company' && (
                  <div className="space-y-6">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="text-xl font-semibold text-gray-900 mb-2">{enhancedJob.company}</h3>
                        <p className="text-gray-700 mb-3">{enhancedJob.companyInfo.description}</p>
                        
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <span className="text-gray-500">Industry:</span>
                            <span className="ml-2 font-medium">{enhancedJob.companyInfo.industry}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Company Size:</span>
                            <span className="ml-2 font-medium">{enhancedJob.companyInfo.size}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Founded:</span>
                            <span className="ml-2 font-medium">{enhancedJob.companyInfo.founded}</span>
                          </div>
                          <div>
                            ...
                            <a 
                              ...
                              className="ml-2 text-purple-600 hover:text-purple-700 font-medium flex items-center"
                            >
                    ...
                    {enhancedJob.employeeTestimonials && enhancedJob.employeeTestimonials.length > 0 && (
                      <div>
                        <h4 className="text-lg font-semibold text-gray-900 mb-4">What Employees Say</h4>
                        <div className="space-y-4">
                          {enhancedJob.employeeTestimonials.map((testimonial, index) => (
                    ...
                    <div>
                      <h4 className="text-lg font-semibold text-gray-900 mb-3">Company Values</h4>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {['Innovation', 'Collaboration', 'Growth'].map((value, index) => (
                          <div key={index} className="text-center p-4 bg-purple-50 rounded-lg">
                            ...
                            <h5 className="font-medium text-gray-900">{value}</h5>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                    <div className="text-center">
                      ...
                      <p className="text-gray-600 mb-6">
                        This will take you to the original job posting to complete your application.
                      </p>
                      ...
                      <div className="bg-gray-50 p-6 rounded-lg mb-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <span className="ml-2 font-medium">{enhancedJob.title}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Company:</span>
                            <span className="ml-2 font-medium">{enhancedJob.company}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Location:</span>
                            <span className="ml-2 font-medium">{enhancedJob.location}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Salary:</span>
                            <span className="ml-2 font-medium">{safeFormatSalary(enhancedJob.salaryMin, enhancedJob.salaryMax)}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      {enhancedJob.applicationDeadline && (
                        <div className="flex items-center justify-center bg-yellow-50 border border-yellow-200 p-4 rounded-lg mb-6">
                          <ExclamationTriangleIcon className="h-5 w-5 text-yellow-600 mr-2" />
                          <span className="text-yellow-800 text-sm">
                            Application deadline: {safeDateFormat(enhancedJob.applicationDeadline, { 
                              year: 'numeric', 
                              month: 'short', 
                              day: 'numeric' 
                            }) || 'Not specified'}
                      ...
                      <button
                        ...
                        className={`inline-flex items-center justify-center px-8 py-4 rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 text-white font-semibold text-lg hover:from-purple-700 hover:to-blue-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 ${
                          applyState.isLoading ? 'opacity-50 cursor-not-allowed transform-none' : ''
                        }`}
                      >
                        {applyState.isLoading ? (
                          <>
                            <LoadingSpinner size="sm" variant="spinner" color="white" className="mr-2" />
                            Applying...
                          </>
                        ) : (
                          <>
                    ...
                    <div className="border-t pt-6">
                      <h4 className="font-medium text-gray-900 mb-3">Tips for Your Application</h4>
                      <ul className="space-y-2 text-sm text-gray-700">
                        ...
                        <li className="flex items-start">
                          <div className="w-2 h-2 bg-purple-500 rounded-full mr-3 mt-2 flex-shrink-0"></div>
                          Write a compelling cover letter that shows your interest in the role
                        </li>
                        <li className="flex items-start">
                          <div className="w-2 h-2 bg-purple-500 rounded-full mr-3 mt-2 flex-shrink-0"></div>
                          Research the company and mention specific reasons you want to work there
                        </li>
                        <li className="flex items-start">
                          <div className="w-2 h-2 bg-purple-500 rounded-full mr-3 mt-2 flex-shrink-0"></div>
                          Follow up appropriately after submitting your application
                        </li>
                      </ul>
                    </div>
                  </div>
          ...
          <div className="border-t bg-gray-50 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              {[1, 2].map((_, index) => (
                <div key={index} className="bg-white p-3 rounded-lg border border-gray-200 hover:border-purple-300 transition-colors cursor-pointer">
                  <h5 className="font-medium text-gray-900 text-sm mb-1">Similar Position {index + 1}</h5>
                  <p className="text-gray-600 text-xs mb-1">Company Name</p>
                  <p className="text-gray-500 text-xs">Location • Job Type</p>
                </div>
              ))}
            </div>
          </div>
