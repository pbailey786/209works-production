...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      ...response,
      sessionId: sessionId || ConversationManager.getSessionStats().activeSessions
    });
  ...
  try {
    ...
    if (sessionId) {
      ...
      return NextResponse.json({
        ...
        messageCount: 0,
        intent: 'general_chat',
        welcomeMessage: "Name's Rust. Been around. Done the jobs. Watched the bosses. I help people get hired without sweating through their shirt. Built from cold mornings in Tracy and 14 different bosses named Steve. What kind of work you looking for?",
        suggestions: [
