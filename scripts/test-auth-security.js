...
async function testAuthenticationSecurity() {
  ...
  try {
    ...
    console.log(
      `✅ Created security log. Total logs for user: ${securityLogs.length}`
    );
    console.log(
      `   Latest log: ${securityLogs[0]?.event} at ${securityLogs[0]?.createdAt}`
    );
    ...
    console.log(
      `✅ User now has ${userAfterFailedAttempts.failedLoginAttempts} failed attempts`
    );
    ...
    console.log(
      `✅ Session version incremented: ${originalVersion} → ${updatedUser.sessionVersion}`
    );
