...
function fixCorruptedImports(content, filePath) {
  ...
  content = content.replace(
    /import\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\}\s*from\s*'([^']+)';'/g,
    "import $1 from '$2';"
  );
  ...
  content = content.replace(
    /import\s+([a-zA-Z_$][a-zA-Z0-9_$]*),\s*\{\s*([^}]+)\s*\}\s*\}\s*\}\s*from\s*'([^']+)';'/g,
    "import $1, { $2 } from '$3';"
  );
  content = content.replace(
  ...
  content = content.replace(
    /import\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\}\s*\}\s*\}\s*from/g,
    'import $1 from'
  );
...
function fixFile(filePath) {
  try {
    ...
    const { content: newContent, hasChanges } = fixCorruptedImports(
      content,
      filePath
    );
