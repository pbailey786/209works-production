...
export async function createJobApplicationSafe(data: {
  ...
  return safeTransaction(async (tx) => {
    ...
    if (job.status !== 'active') {
      throw new TransactionError('Job is not accepting applications', 'JOB_INACTIVE');
    }
    
    if (job.expiresAt && job.expiresAt < new Date()) {
      throw new TransactionError('Application deadline has passed', 'JOB_EXPIRED');
    }
    ...
    if (!user || user.role !== 'jobseeker') {
      throw new TransactionError('Invalid user or insufficient permissions', 'USER_INVALID');
    }
    ...
    try {
      ...
      if ((error as any).code === 'P2002') {
        throw new TransactionError('You have already applied to this job', 'DUPLICATE_APPLICATION');
      }
      throw error;
    }
  }, {
...
export async function createUserWithCompanySafe(data: {
  ...
  return safeTransaction(async (tx) => {
    ...
    if (existingUser) {
      throw new TransactionError('User with this email already exists', 'EMAIL_EXISTS');
    }
    ...
    if (data.companyData && !companyId) {
      ...
      if (existingByName) {
        throw new TransactionError('Company with this name already exists', 'COMPANY_NAME_EXISTS');
      }
      
      if (existingBySlug) {
        throw new TransactionError('Company with this slug already exists', 'COMPANY_SLUG_EXISTS');
      }
    ...
    if (companyId) {
      ...
      if (!company.isActive) {
        throw new TransactionError('Company is not active', 'COMPANY_INACTIVE');
      }
    }
...
export async function createJobSafe(data: {
  ...
  return safeTransaction(async (tx) => {
    ...
    if (existingJob) {
      throw new TransactionError('Job with this URL already exists', 'DUPLICATE_URL');
    }
 ...
 */
export async function createJobsBatchSafe(jobs: Array<{
  title: string;
  company: string;
  companyId?: string;
  description: string;
  ...
  return safeTransaction(async (tx) => {
    ...
    if (jobs.length > 1000) {
      throw new TransactionError('Batch size too large (max 1000)', 'BATCH_TOO_LARGE');
    }
    ...
    if (urls.length !== uniqueUrls.size) {
      throw new TransactionError('Duplicate URLs found in batch', 'DUPLICATE_URLS_IN_BATCH');
    }
    ...
    if (companyIds.length > 0) {
      ...
      const missingCompanyIds = companyIds.filter(id => !foundCompanyIds.has(id));
 ...
 */
export async function updateWithOptimisticLock<T extends { id: string; updatedAt: Date }>(
  model: any,
  id: string,
