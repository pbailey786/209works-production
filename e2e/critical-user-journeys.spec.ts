...
// Helper functions
async function signUp(page: Page, userData: typeof testUser, role: 'job_seeker' | 'employer' = 'job_seeker') {
...
async function postJob(page: Page, jobData: typeof testJob) {
  ...
  await page.selectOption('[data-testid="experience-level"]', jobData.experienceLevel);
  ...
  for (const benefit of jobData.benefits) {
    await page.click(`[data-testid="benefit-${benefit.toLowerCase().replace(/\s+/g, '-')}"]`);
  }
...
test.describe('Critical User Journeys', () => {
  ...
  test.describe('Job Seeker Journey', () => {
    test('Complete job seeker registration and job search flow', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="job-saved-message"]')).toBeVisible();
    ...
    test('JobsGPT conversation flow', async ({ page }) => {
      ...
      await page.fill('[data-testid="chat-input"]', 'I\'m looking for software engineering jobs in the 209 area');
      ...
      await page.fill('[data-testid="chat-input"]', 'What about remote opportunities?');
      await page.click('[data-testid="send-button"]');
    ...
    test('Profile management and resume upload', async ({ page }) => {
      ...
      await page.fill('[data-testid="bio-input"]', 'Experienced software developer with 5+ years...');
      await page.fill('[data-testid="linkedin-url"]', 'https://linkedin.com/in/testuser');
      await page.click('[data-testid="save-profile"]');
      
      // 3. Upload resume
      await page.setInputFiles('[data-testid="resume-upload"]', 'test-files/sample-resume.pdf');
      await expect(page.locator('[data-testid="upload-success"]')).toBeVisible();
      
      // 4. Verify resume analysis
      await expect(page.locator('[data-testid="resume-score"]')).toBeVisible();
      await expect(page.locator('[data-testid="resume-suggestions"]')).toBeVisible();
    });
  });
  ...
  test.describe('Employer Journey', () => {
    test('Complete employer registration and job posting flow', async ({ page }) => {
      ...
      await page.fill('[data-testid="company-description"]', 'We are a leading technology company...');
    ...
    test('Applicant tracking and management', async ({ page }) => {
      ...
      await page.fill('[data-testid="status-notes"]', 'Great candidate, moving to next round');
      await page.click('[data-testid="save-status"]');
      
      // 6. Send message to applicant
      await page.click('[data-testid="message-applicant"]');
      await page.fill('[data-testid="message-content"]', 'Thank you for your application...');
      await page.click('[data-testid="send-message"]');
      
      // 7. Verify status updated
      await expect(page.locator('[data-testid="status-badge"]')).toContainText('Interviewed');
    });
  });
  ...
  test.describe('Cross-Platform Features', () => {
    test('Social features and networking', async ({ page }) => {
      ...
      await page.fill('[data-testid="review-content"]', 'I really enjoyed working here...');
      ...
      await page.fill('[data-testid="post-title"]', 'Tips for software engineering interviews?');
      await page.fill('[data-testid="post-content"]', 'I have an interview coming up...');
