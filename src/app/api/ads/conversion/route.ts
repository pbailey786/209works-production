  ...
  async (req, context) => {
    ...
    const conversion = await prisma.adConversion.create({
      data: {
        ...
        metadata: body.customEvent ? { customEvent: body.customEvent } : undefined,
      },
      select: {
        id: true,
        conversionType: true,
...
async function calculateConversionFunnel(adId: string, performance: any) {
  ...
  return {
    byType: conversionsByType.map(item => ({
      ...
      averageValue: item._count.conversionType > 0 ? Number(item._sum.conversionValue || 0) / item._count.conversionType : 0,
    })),
