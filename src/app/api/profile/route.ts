...
export async function PATCH(req: NextRequest) {
  ...
  if (workAuthorization !== undefined) updateData.workAuthorization = workAuthorization;
  if (educationExperience !== undefined) updateData.educationExperience = educationExperience;
...
export async function POST(req: NextRequest) {
  const token = await getToken({ req, secret: process.env.NEXTAUTH_SECRET });
  if (!token || !token.email || !token.sub) { // Ensure token.sub (userId) is present
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  ...
  if (profilePictureFile) {
    ...
    if (!allowedImageTypes.includes(profilePictureFile.type)) {
      return NextResponse.json({ error: "Invalid image type (JPEG, PNG, GIF, WEBP only)" }, { status: 400 });
    }
    const bytes = await profilePictureFile.arrayBuffer();
    const buffer = Buffer.from(bytes);
    const originalFilename = profilePictureFile.name.replace(/[^a-zA-Z0-9._-]/g, '');
    const timestamp = Date.now();
    const uniqueFilename = `${timestamp}-${originalFilename}`;
    const picDir = path.join(process.cwd(), "public", "profile-pictures", userId);
