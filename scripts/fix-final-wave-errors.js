...
function fixFinalWaveErrors(content, filePath) {
  ...
  if (
    ...
    !content.includes("from '@/hooks/use-toast'")
  ) {
    const lines = content.split('\n');
    const firstImportIndex = lines.findIndex(line =>
      line.trim().startsWith('import')
  ...
  if (
    ...
    !content.includes("from '@hookform/resolvers/zod'")
  ) {
    const lines = content.split('\n');
    const firstImportIndex = lines.findIndex(line =>
      line.trim().startsWith('import')
...
function fixFile(filePath) {
  try {
    ...
    const { content: newContent, hasChanges } = fixFinalWaveErrors(
      content,
      filePath
    );
