...
function round5AggressiveCleanup(content, filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      if (
        trimmed.match(
          /^(const|let|var|import|export|return|throw|break|continue|delete|typeof)\s/
        ) ||
  ...
  content = content.replace(
    /\bfunction\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g,
    'function $1'
  );
...
function fixFile(filePath) {
  try {
    ...
    const { content: newContent, hasChanges } = round5AggressiveCleanup(
      content,
      filePath
    );
...
function main() {
  ...
  if (fixedCount > 0) {
    ...
    console.log(
      '💡 Run "npm run type-check" to see the maximum impact improvements.'
    );
  } else {
