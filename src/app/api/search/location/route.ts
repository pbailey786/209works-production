...
function generateLocationInsights(
  ...
  const jobsWithDistance = jobs.filter(job => 
    job && 
  ...
  const jobTypes = Object.entries(jobTypeCounts)
    ...
    .map(([type, count]) => ({ 
      type, 
      count, 
      percentage: jobs.length > 0 ? Math.round((count / jobs.length) * 100 * 10) / 10 : 0 
    }));
  ...
  const validSalaries = jobs
    ...
    .map(job => {
      ...
      return typeof salary === 'number' && !isNaN(salary) && isFinite(salary) && salary > 0 
        ? salary 
        : null;
    })
    .filter(salary => salary !== null);
  
  const salaryRange = validSalaries.length > 0 ? {
    min: Math.min(...validSalaries),
    max: Math.max(...validSalaries),
    average: Math.round((validSalaries.reduce((sum, salary) => sum + salary, 0) / validSalaries.length) * 100) / 100,
  } : null;
  
  // Top companies with null checks
  const companyCounts: Record<string, number> = {};
  jobs.forEach(job => {
    if (job && job.company && typeof job.company === 'string' && job.company.trim()) {
  ...
  return {
    averageDistance: averageDistance ? Math.round(averageDistance * 10) / 10 : null,
    jobTypes,
    salaryRange,
    topCompanies,
