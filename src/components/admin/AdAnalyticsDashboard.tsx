...
export default function AdAnalyticsDashboard({ adId }: AdAnalyticsProps) {
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/ads/${adId}/analytics?range=${dateRange}`);
  ...
  const exportData = async () => {
    try {
      const response = await fetch(`/api/ads/${adId}/analytics/export?range=${dateRange}`);
              ...
              <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="font-medium">
                    {new Date(data.ad.startDate).toLocaleDateString()} - {new Date(data.ad.endDate).toLocaleDateString()}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Budget</p>
                  <p className="font-medium">{formatCurrency(data.ad.budget || 0)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Spent</p>
                  <p className="font-medium">{formatCurrency(data.ad.spent || 0)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Remaining</p>
                  <p className="font-medium">{formatCurrency((data.ad.budget || 0) - (data.ad.spent || 0))}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{formatNumber(data.metrics.impressions)}</p>
              </div>
            ...
            <div className="mt-2 flex items-center">
              ...
              <span className={`text-sm ${data.performance.trend === 'up' ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{formatNumber(data.metrics.clicks)}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{formatNumber(data.metrics.conversions)}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{formatCurrency(data.metrics.revenue)}</p>
              </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatCurrency(data.metrics.costPerClick)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Cost Per Conversion</span>
                  <span className="font-medium">{formatCurrency(data.metrics.costPerConversion)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Total Spent</span>
                  <span className="font-medium">{formatCurrency(data.ad.spent || 0)}</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <p className="font-medium">{data.performance.bestPerformingDay}</p>
                  <p className="text-sm text-gray-600 mt-4">Current trend:</p>
                  <div className="flex items-center">
                    ...
                    <span className={data.performance.trend === 'up' ? 'text-green-600' : 'text-red-600'}>
                      {data.performance.trend === 'up' ? 'Improving' : 'Declining'}
                    </span>
                  </div>
                </div>
              </CardContent>
                  ...
                  {data.demographics.locations.map((location, index) => (
                    <div key={index} className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-medium">{formatNumber(location.clicks)} clicks</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {data.performance.recommendations.map((recommendation, index) => (
