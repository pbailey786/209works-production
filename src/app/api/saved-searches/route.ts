...
export async function GET(req: NextRequest) {
  try {
    ...
    const { limit, offset, active } = getSavedSearchesSchema.parse({
    ...
    // Get saved searches
    const [savedSearches, totalCount] = await Promise.all([
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, query, filters, alertEnabled } = createSavedSearchSchema.parse(body);
    ...
    // Check user's saved search limit (e.g., 10 for free users)
    ...
    if (savedSearchCount >= 10) {
      return NextResponse.json(
        { error: 'You have reached the maximum number of saved searches (10)' },
        { status: 400 }
      );
    }
