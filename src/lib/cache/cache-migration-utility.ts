...
interface MigrationStatus {
  phase: 'preparation' | 'migration' | 'validation' | 'cleanup' | 'completed' | 'failed';
  startTime: number;
  endTime?: number;
  progress: number;
  errors: string[];
...
export class CacheMigrationManager {
  ...
  private async prepareMigration(): Promise<void> {
    try {
      ...
      console.log(`Found ${cacheKeys.length} cache keys to migrate out of ${allKeys.length} total keys`);
  ...
  private async validateCurrentCache(keys: string[]): Promise<void> {
    ...
    if (invalidKeys > validKeys * 0.1) { // More than 10% invalid
      this.addWarning(`High number of invalid cache entries: ${invalidKeys}/${validKeys + invalidKeys}`);
    }
  }
  ...
  private async prepareRollbackData(keys: string[]): Promise<void> {
    ...
    for (const key of keys.slice(0, 1000)) { // Limit rollback data
      try {
        const value = await this.redis.get(key);
        const ttl = await this.redis.ttl(key);
        ...
        this.addWarning(`Failed to prepare rollback data for key ${key}: ${error}`);
      }
    }
  ...
  async rollback(): Promise<void> {
    ...
    try {
      ...
      const atomicKeys = newKeys.filter((key: string) => key.includes('version:') || key.includes('lock:'));
...
export const MigrationUtils = {
   ...
   */
  async runMigration(config: Partial<MigrationConfig> = {}): Promise<MigrationStatus> {
  ...
  async validateReadiness(): Promise<{
    ...
    try {
      ...
      if (cacheKeys.length > 10000) {
        recommendations.push('Large number of cache keys detected. Consider running migration during low traffic period.');
      }
      ...
      if (memoryUsage > 500 * 1024 * 1024) { // 500MB
        recommendations.push('High memory usage detected. Monitor memory during migration.');
      }
