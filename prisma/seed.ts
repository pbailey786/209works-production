...
async function main() {
  ...
  const testJobs = [];
  const companies = ['TechCorp', 'InnovateLabs', 'DataSystems', 'CloudWorks', 'DevStudio', 'StartupXYZ', 'MegaTech', 'CodeCraft', 'WebSolutions', 'AppFactory'];
  const jobTitles = ['Software Engineer', 'Frontend Developer', 'Backend Developer', 'Full Stack Developer', 'DevOps Engineer', 'Data Scientist', 'Product Manager', 'UX Designer', 'QA Engineer', 'Mobile Developer'];
  const locations = ['San Francisco, CA', 'New York, NY', 'Austin, TX', 'Seattle, WA', 'Boston, MA', 'Denver, CO', 'Chicago, IL', 'Los Angeles, CA', 'Remote', 'Portland, OR'];
