...
async function testPhase2Chatbot() {
  ...
  try {
    ...
    const companiesFromJobs =
      await CompanyKnowledgeService.getCompaniesFromJobs();
    ...
    if (companiesFromJobs.length > 0) {
      ...
      const companyInfo =
        await CompanyKnowledgeService.getCompanyInfo(testCompany);
      if (companyInfo) {
        console.log(
          `✅ Company knowledge retrieved: ${companyInfo.knowledgeEntries.length} entries`
        );
    ...
    for (const message of companyTestMessages) {
      ...
      try {
        ...
        if (response.jobRecommendations?.length) {
          console.log(
            `🔍 Found ${response.jobRecommendations.length} related jobs`
          );
        }

        // Check if company info was included
        const hasCompanyInfo =
          response.reply.toLowerCase().includes('culture') ||
        ...
        if (hasCompanyInfo) {
          ...
          console.log(
            `⚠️  Response may be generic (no specific company info detected)`
          );
        }
      } catch (error) {
        if (error instanceof Error) {
          ...
          console.error(
            '❌ Error processing message: An unknown error occurred'
          );
        }
      }
    }
    ...
    if (companiesFromJobs.length > 0) {
      ...
      const searchTests = [
        {
          query: 'culture',
          category: undefined as CompanyKnowledgeCategory | undefined,
        },
        {
          query: 'benefits',
          category: undefined as CompanyKnowledgeCategory | undefined,
      ...
      for (const test of searchTests) {
        ...
        results.forEach(result => {
          console.log(
            `   - [${result.category}] ${result.title} (${result.views} views)`
          );
        });
      }
    }
      ...
      if (companyInfo) {
        ...
        if (analytics) {
          ...
          analytics.byCategory.forEach((cat: any) => {
            console.log(
              `     - ${cat.category}: ${cat.entryCount} entries, ${cat.totalViews} views`
            );
          });
        }
      }
...
async function ensureTestData() {
  ...
  try {
    ...
    const companyInfo =
      await CompanyKnowledgeService.getCompanyInfo(testCompanyName);

    if (!companyInfo) {
      console.log(
        `⚠️ Test company '${testCompanyName}' not found. Run setup-company-knowledge.ts again.`
      );
      return false;
    }

    if (
      !companyInfo.knowledgeEntries ||
      companyInfo.knowledgeEntries.length === 0
    ) {
      console.log(
        `⚠️ No knowledge entries found for '${testCompanyName}'. Run setup-company-knowledge.ts again.`
      );
      return false;
    }
