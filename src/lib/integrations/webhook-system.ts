...
export interface WebhookEventType {
  ...
  category: 'job' | 'application' | 'user' | 'payment' | 'system';
}
...
export class WebhookSystem {
  ...
  private eventTypes: Map<string, WebhookEventType> = new Map();
  ...
  static getInstance(): WebhookSystem {
    if (!this.instance) {
      this.instance = new WebhookSystem();
    }
    return this.instance;
  }

  /**
   * Initialize supported webhook event types
   */
  private initializeEventTypes() {
    const eventTypes: WebhookEventType[] = [
      ...
      {
        name: 'job.updated',
        description: 'A job posting has been updated',
        category: 'job',
        schema: {
          type: 'object',
          properties: {
      ...
      {
        name: 'job.expired',
        description: 'A job posting has expired',
        category: 'job',
        schema: {
          type: 'object',
          properties: {
  ...
  async emitEvent(params: {
    ...
    const { type, data, source = '209works', userId, region } = params;
    ...
    try {
      ...
      // Find active webhook endpoints for this event type
      const webhookEndpoints = await this.getActiveWebhooks(type, userId);

      // Create deliveries for each webhook endpoint
      for (const webhook of webhookEndpoints) {
        await this.createDelivery(event, webhook);
      }

      // Log event emission
      AuditLogger.log({
        action: 'webhook_event_emitted',
        ...
        details: {
          eventType: type,
          webhookCount: webhookEndpoints.length,
        },
      });
   ...
   */
  private async createDelivery(event: WebhookEvent, webhook: any): Promise<WebhookDelivery> {
    const payload = this.createPayload(event, webhook);
    const headers = this.createHeaders(event, webhook);
   ...
   */
  private async startDeliveryProcessor() {
    setInterval(async () => {
  ...
  private async processDelivery(delivery: WebhookDelivery): Promise<void> {
    try {
      ...
      if (response.ok) {
        // Successful delivery
        await this.markDeliverySuccess(delivery, response, responseBody);
      } else {
        // Failed delivery
        await this.markDeliveryFailure(delivery, response, responseBody);
      }

    } catch (error) {
      // Network or other error
      await this.markDeliveryError(delivery, error);
    }
  }
  ...
  private async markDeliverySuccess(
    ...
    // Reset failure count for webhook endpoint
  ...
  private async markDeliveryFailure(
    ...
    if (shouldRetry) {
      ...
      // Re-queue for retry
      setTimeout(() => {
        this.deliveryQueue.push({ ...delivery, nextRetryAt });
      }, retryDelay);
      ...
      // Increment failure count for webhook endpoint
      await this.incrementWebhookFailureCount(delivery.webhookId);
    }
  }
   ...
   */
  private async markDeliveryError(delivery: WebhookDelivery, error: any): Promise<void> {
    const shouldRetry = delivery.attemptCount < delivery.maxAttempts;
   ...
   * Helper methods
   */
  private async getActiveWebhooks(eventType: string, userId?: string) {
  ...
  private createPayload(event: WebhookEvent, webhook: any) {
    return {
      id: event.id,
      type: event.type,
      data: event.data,
  ...
  private createHeaders(event: WebhookEvent, webhook: any): Record<string, string> {
    const timestamp = Math.floor(Date.now() / 1000);
    const payload = JSON.stringify(this.createPayload(event, webhook));
    const signature = this.generateSignature(payload, webhook.secret, timestamp);

    return {
      'Content-Type': 'application/json',
      'User-Agent': '209Works-Webhooks/1.0',
      'X-209Works-Event': event.type,
      'X-209Works-Delivery': this.generateDeliveryId(),
      'X-209Works-Signature': signature,
      'X-209Works-Timestamp': timestamp.toString(),
    };
  }
