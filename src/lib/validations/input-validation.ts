...
export const enhancedIdSchema = z.string()
...
export const enhancedUrlSchema = z.string()
  .url('Invalid URL format')
  .max(VALIDATION_LIMITS.MAX_MEDIUM_STRING_LENGTH)
  .transform(sanitizeString);

export const enhancedSearchQuerySchema = z.string()
  .max(VALIDATION_LIMITS.MAX_MEDIUM_STRING_LENGTH)
  .transform(sanitizeSearchQuery);

export const enhancedTextContentSchema = z.string()
  .max(VALIDATION_LIMITS.MAX_MESSAGE_LENGTH)
  .transform(sanitizeHtml);

export const enhancedArraySchema = <T>(itemSchema: z.ZodSchema<T>, maxLength: number = VALIDATION_LIMITS.MAX_ARRAY_LENGTH) =>
  z.array(itemSchema)
...
export const paginationSchema = z.object({
  page: z.coerce.number()
    ...
    .max(VALIDATION_LIMITS.MAX_PAGE_NUMBER, `Page cannot exceed ${VALIDATION_LIMITS.MAX_PAGE_NUMBER}`)
    .default(1),
  limit: z.coerce.number()
    .int('Limit must be an integer')
    .min(VALIDATION_LIMITS.MIN_PAGE_SIZE, `Limit must be at least ${VALIDATION_LIMITS.MIN_PAGE_SIZE}`)
    .max(VALIDATION_LIMITS.MAX_PAGE_SIZE, `Limit cannot exceed ${VALIDATION_LIMITS.MAX_PAGE_SIZE}`)
    .default(20),
...
export const fileValidationSchema = z.object({
  filename: enhancedStringSchema(1, VALIDATION_LIMITS.MAX_SHORT_STRING_LENGTH)
    .refine((val) => /^[a-zA-Z0-9._-]+$/.test(val), 'Filename contains invalid characters'),
  size: z.number()
...
export const searchFiltersSchema = z.object({
  ...
  category: enhancedStringSchema(1, VALIDATION_LIMITS.MAX_SHORT_STRING_LENGTH).optional(),
  location: enhancedStringSchema(1, VALIDATION_LIMITS.MAX_SHORT_STRING_LENGTH).optional(),
  dateFrom: z.coerce.date().optional(),
  dateTo: z.coerce.date().optional(),
  sortBy: z.enum(['relevance', 'date', 'title', 'company']).default('relevance'),
...
export const rateLimitConfigSchema = z.object({
  ...
  identifier: enhancedStringSchema(1, VALIDATION_LIMITS.MAX_SHORT_STRING_LENGTH).optional(),
});
...
export function validateTotalContentLength(
  ...
  if (totalLength > maxTotalLength) {
    throw new Error(`${fieldName} exceeds maximum total length of ${maxTotalLength} characters`);
  }
}
...
export function validateInput<T>(
  ...
  const { sanitize = true, checkSql = true, checkScript = true, checkPath = false } = options || {};
  ...
  if (typeof parsed === 'object' && parsed !== null) {
    const checkStringValue = (value: any, key?: string) => {
      if (typeof value === 'string') {
        ...
        if (checkPath && (key?.toLowerCase().includes('path') || key?.toLowerCase().includes('file'))) {
          validateNoPathTraversal(value);
        }
      } else if (Array.isArray(value)) {
        value.forEach((item, index) => checkStringValue(item, `${key}[${index}]`));
      } else if (typeof value === 'object' && value !== null) {
        Object.entries(value).forEach(([k, v]) => checkStringValue(v, k));
      }
    };
    
    Object.entries(parsed).forEach(([key, value]) => checkStringValue(value, key));
  }
