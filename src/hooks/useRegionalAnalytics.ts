...
export function useRegionalAnalytics(region?: string) {
  ...
  const getPageType = (path: string): string => {
    if (path === '/') return 'home';
    if (path.startsWith('/jobs/') && path.split('/').length === 3) return 'job_detail';
  ...
  const trackJobViewEvent = useCallback((jobData: JobData, source?: string) => {
  ...
  const trackJobAlertCreation = useCallback((alertData: {
    keywords?: string[];
    region?: string;
    jobType?: string;
    location?: string;
  ...
  const trackSearchFilterUsage = useCallback((filters: SearchFilters) => {
  ...
  const trackEmailAlertSubscription = useCallback((alertData: {
    frequency?: string;
    categories?: string[];
    location?: string;
  }) => {
  ...
  const trackEventWithRegion = useCallback((eventName: string, properties?: Record<string, any>) => {
