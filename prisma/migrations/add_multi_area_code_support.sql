...
-- ============================================================================
-- PART 5: UPDATE TABLE STATISTICS AND VALIDATE MIGRATION
-- ============================================================================

-- Update table statistics for better query planning
ANALYZE "Job";

-- Validate that all jobs have area codes
DO $$
DECLARE
  jobs_without_area_codes INTEGER;
BEGIN
  SELECT COUNT(*) INTO jobs_without_area_codes
  FROM "Job" 
  WHERE "area_codes" IS NULL OR array_length("area_codes", 1) IS NULL;
  
  IF jobs_without_area_codes > 0 THEN
    RAISE WARNING 'Found % jobs without area codes - this should be investigated', jobs_without_area_codes;
  ELSE
    RAISE NOTICE 'All jobs have area codes assigned - migration validation successful';
  END IF;
END $$;

-- ============================================================================
-- PART 6: MIGRATION COMPLETION LOG
-- ============================================================================
...
DECLARE
  total_jobs INTEGER;
  jobs_with_cities INTEGER;
BEGIN
  SELECT COUNT(*) INTO total_jobs FROM "Job";
  SELECT COUNT(*) INTO jobs_with_cities FROM "Job" WHERE "city" IS NOT NULL;
  
  RAISE NOTICE 'Multi-area-code network migration completed successfully';
  RAISE NOTICE 'Total jobs processed: %', total_jobs;
  RAISE NOTICE 'Jobs with city data: %', jobs_with_cities;
  RAISE NOTICE 'Added 5 new indexes for area code queries';
