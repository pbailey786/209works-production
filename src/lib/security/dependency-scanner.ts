...
export class DependencyScanner {
      ...
      for (const vuln of snykData.vulnerabilities) {
        const vulnerability: VulnerabilityInfo = {
          ...
          recommendation: vuln.fixedIn
            ? `Update to version ${vuln.fixedIn}`
            : 'No fix available',
  ...
  private generateRecommendations(
    ...
    if (criticalCount > 0) {
      recommendations.push(
        `🚨 ${criticalCount} critical vulnerabilities found - immediate action required`
      );
    }

    if (highCount > 0) {
      recommendations.push(
        `⚠️ ${highCount} high severity vulnerabilities found - update soon`
      );
    }
    ...
    recommendations.push(
      'Review all vulnerabilities and update dependencies regularly'
    );
    recommendations.push(
  ...
  shouldFailBuild(scanResult: ScanResult): boolean {
    return SECURITY_CONFIG.failOnSeverity.some(
      severity =>
