...
export const createJobSchema = z.object({
  ...
  salaryMin: z.union([
    ...
    z.string().transform((val) => val === '' ? undefined : parseInt(val, 10)).optional()
  ]).optional(),
  salaryMax: z.union([
    z.number().int().min(0, 'Salary must be positive'),
    z.string().transform((val) => val === '' ? undefined : parseInt(val, 10)).optional()
  ]).optional(),
  categories: z.array(z.string().max(50, 'Category name too long')).max(10, 'Too many categories').optional(),
  ...
  const minSalary = typeof data.salaryMin === 'number' ? data.salaryMin : 
    (typeof data.salaryMin === 'string' && data.salaryMin ? parseInt(data.salaryMin) : 0);
  const maxSalary = typeof data.salaryMax === 'number' ? data.salaryMax : 
    (typeof data.salaryMax === 'string' && data.salaryMax ? parseInt(data.salaryMax) : 0);
...
export const updateJobSchema = z.object({  title: z.string().min(1, 'Title is required').max(200, 'Title too long').optional(),  description: z.string().min(10, 'Description must be at least 10 characters').max(5000, 'Description too long').optional(),  company: z.string().min(1, 'Company is required').max(100, 'Company name too long').optional(),  location: z.string().min(1, 'Location is required').max(100, 'Location too long').optional(),  type: jobTypeSchema.optional(),  salaryMin: z.number().int().min(0, 'Salary must be positive').optional(),  salaryMax: z.number().int().min(0, 'Salary must be positive').optional(),  categories: z.array(z.string().max(50, 'Category name too long')).max(10, 'Too many categories').optional(),  source: z.string().max(100, 'Source too long').optional(),  url: urlSchema,  postedAt: z.string().datetime('Invalid date format').optional(),}).refine(data => {  if (data.salaryMin && data.salaryMax && data.salaryMin > data.salaryMax) {    return false;  }  return true;}, {  message: 'Minimum salary cannot be greater than maximum salary',  path: ['salaryMin'],});

export const jobSearchSchema = z.object({
  page: z.coerce.number().int().min(1, 'Page must be at least 1').max(1000, 'Page too high').optional(),
  limit: z.coerce.number().int().min(1, 'Limit must be at least 1').max(100, 'Limit too high').optional(),
  type: jobTypeSchema.optional(),
  location: z.string().max(100, 'Location too long').optional(),
  company: z.string().max(100, 'Company name too long').optional(),
  sortBy: z.enum(['postedAt', 'createdAt', 'salaryMin', 'salaryMax'], {
...
export const updateUserSchema = z.object({
  ...
  preferredJobTypes: z.array(jobTypeSchema).max(5, 'Too many preferred job types').optional(),
  skills: z.array(z.string().min(1, 'Skill cannot be empty').max(50, 'Skill name too long')).max(20, 'Too many skills').optional(),
  workAuthorization: z.string().max(100, 'Work authorization too long').optional(),
  educationExperience: z.string().max(2000, 'Education experience too long').optional(),
...
export const userApplicationsSchema = z.object({
  page: z.coerce.number().int().min(1, 'Page must be at least 1').max(1000, 'Page too high').optional(),
  limit: z.coerce.number().int().min(1, 'Limit must be at least 1').max(50, 'Limit too high').optional(),
