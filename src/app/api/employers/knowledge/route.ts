...
export async function GET(req: NextRequest) {
  try {
    ...
    const companyInfo = await CompanyKnowledgeService.getCompanyInfo(user.company.name);
    ...
    // Get analytics
    const analytics = await CompanyKnowledgeService.getCompanyKnowledgeAnalytics(user.company.id);
    ...
    return NextResponse.json({
      ...
      categories: [
        'culture', 'benefits', 'hiring_process', 'perks', 'career_growth',
        'work_environment', 'compensation', 'remote_policy', 'diversity_inclusion',
        'company_values', 'interview_process', 'onboarding', 'training', 'general_info'
      ]
    });
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { entryId, category, title, content, keywords, priority } = await req.json();
    ...
    // Update the entry
    const updatedEntry = await CompanyKnowledgeService.updateCompanyKnowledge(entryId, {
      category,
      title,
      content,
      keywords,
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // Delete the entry
    const deleted = await CompanyKnowledgeService.deleteCompanyKnowledge(entryId);
