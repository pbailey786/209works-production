...
} = require('../src/lib/services/job-posting-credits');
...
async function testServiceVersion() {
  ...
  try {
    ...
    console.log(
      '- getUserCredits:',
      typeof JobPostingCreditsService.getUserCredits
    );
    ...
    const hasOldDebugLogging =
      serviceSource.includes('Found credits:') ||
    ...
    if (hasOldDebugLogging) {
      ...
      console.log(
        'Service source preview:',
        serviceSource.substring(0, 500) + '...'
      );
    } else {
