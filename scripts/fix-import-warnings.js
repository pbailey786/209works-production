...
const importCorrections = {
  ...
  Table: {
    from: ['@/components/ui/dialog', 'lucide-react'],
  ...
  // API and middleware
  withAPIMiddleware: {
    from: ['@/components/ui/card', '@/lib/middleware/api'],
    to: '@/lib/middleware/api-middleware',
  },
  routeParamsSchemas: {
    from: ['@/components/ui/card', '@/lib/errors/api-errors'],
    to: '@/lib/errors/api-errors',
  },
  updateUserSchema: {
    from: ['@/components/ui/card'],
    to: '@/lib/validations/api',
  },
  userSearchQuerySchema: {
    from: ['@/components/ui/card', '@/lib/validations/api'],
    to: '@/lib/validations/api',
  },
  ...
  prisma: {
    from: [
      '@/lib/errors/api-errors',
      '@/lib/utils/api-response',
      '@/lib/cache/redis',
      '@/lib/middleware/ai-security',
    ],
    to: '@/lib/database/prisma',
  },
  ...
  emailService: {
    from: ['@/components/ui/card'],
    to: '@/lib/email/email-service',
  },
  emailSecurityValidator: {
    from: ['@/components/ui/card'],
    to: '@/lib/email/security',
  },
  SecurityLogger: {
    from: [
      '@/components/ui/card',
      './email/security',
      '@/lib/monitoring/error-monitor',
    ],
    to: '@/lib/monitoring/security-logger',
  },
  EmailHelpers: {
    from: ['@/components/ui/card'],
    to: '@/lib/email/email-helpers',
  },
  emailAgent: {
    from: ['@/components/ui/card'],
    to: '@/lib/agents/email-agent',
  },
  templateManager: {
    from: ['@/components/ui/card'],
    to: '@/lib/email/template-manager',
  },
  TemplateManager: {
    from: ['@/components/ui/card'],
    to: '@/lib/email/template-manager',
  ...
  getDomainConfig: {
    from: ['@/components/ui/card'],
    to: '@/lib/domain/config',
  },

  // Regional services
  RegionalJobService: {
    from: ['@/components/ui/card'],
    to: '@/lib/services/regional-job-service',
  },

  // User services
  UserSearchService: {
    from: ['@/components/ui/card'],
    to: '@/lib/services/user-search',
  },

  // Instagram services
  InstagramAPI: {
    from: ['@/components/ui/card'],
    to: '@/lib/services/instagram-api',
  },

  // Job services
  JobMatchingService: {
    from: ['@/components/ui/card'],
    to: '@/lib/services/job-matching',
  },
  ResumeEmbeddingService: {
    from: ['@/components/ui/card'],
    to: '@/lib/services/resume-embedding',
  },

  // Conversation services
  ConversationManager: {
    from: ['@/components/ui/card'],
    to: '@/lib/conversation-memory',
  },
  ChatbotPrompts: {
    from: ['@/components/ui/card'],
    to: '@/lib/conversation/prompts',
  },

  // Knowledge services
  LocalKnowledgeService: {
    from: ['@/lib/knowledge/company-knowledge'],
    to: '@/lib/conversation/local-knowledge',
  },
  ...
  getUserPermissions: {
    from: ['@/lib/auth/permissions', '@/components/ui/card'],
  ...
  DataIntegrityService: {
    from: ['@/components/ui/card'],
    to: '@/lib/database/data-integrity',
  },
  DuplicateDetectionService: {
    from: ['@/components/ui/card'],
    to: '@/lib/services/duplicate-detection',
  },
  JobQueueService: {
    from: ['@/components/ui/card'],
    to: '@/lib/services/job-queue',
  },
  UserCacheService: {
    from: ['@/lib/errors/api-errors'],
    to: '@/lib/services/user-cache',
  },
  JobCacheService: {
    from: ['@/lib/services/regional-job-service'],
    to: '@/lib/services/job-cache',
  },
  TextProcessor: {
    from: ['@/lib/cache/redis'],
    to: '@/lib/utils/text-processor',
  },
  ShouldIApplyAnalysisService: {
    from: ['@/components/ui/card'],
    to: '@/lib/services/should-i-apply',
  },

  // Stripe and payment services
  validateStripeConfig: {
    from: ['@/components/ui/card'],
    to: '@/lib/stripe/config',
  },
  getStripePriceId: {
    from: ['@/components/ui/card'],
    to: '@/lib/stripe/config',
  ...
  isValidResumeFile: {
    from: ['@/components/ui/card'],
    to: '@/lib/utils/file-validation',
  },
  extractTextFromFile: {
    from: ['@/components/ui/card'],
    to: '@/lib/utils/file-processing',
  },
...
const componentCorrections = {
  DashboardLayout: {
    ...
    to: '@/components/dashboard/DashboardLayout',
  },
  RecentChatsWidget: {
    from: ['@/components/dashboard/JobSeekerWidgets'],
    to: '@/components/dashboard/JobsGPTWidgets',
  },
  SavedSearchesWidget: {
    from: ['@/components/dashboard/JobSeekerWidgets'],
    to: '@/components/dashboard/JobsGPTWidgets',
  ...
  AdPreviewModal: {
    from: ['./AdPreviewModal'],
    to: '@/components/admin/AdPreviewModal',
  },
};
...
function fixImportsInFile(filePath) {
  try {
    ...
    Object.entries(importCorrections).forEach(([importName, correction]) => {
      correction.from.forEach(wrongPath => {
        const patterns = [
          // Single import
          new RegExp(
            `import\\s*\\{\\s*${importName}\\s*\\}\\s*from\\s*['"]${wrongPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"];?`,
            'g'
          ),
          // Multiple imports containing this one
          new RegExp(
            `(import\\s*\\{[^}]*?)\\b${importName}\\b([^}]*\\}\\s*from\\s*['"]${wrongPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"];?)`,
            'g'
          ),
        ];
    ...
    Object.entries(componentCorrections).forEach(
      ([componentName, correction]) => {
        correction.from.forEach(wrongPath => {
          const pattern = new RegExp(
            `import\\s*\\{[^}]*\\b${componentName}\\b[^}]*\\}\\s*from\\s*['"]${wrongPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"];?`,
            'g'
          );
          if (pattern.test(content)) {
            content = content.replace(
              pattern,
              `import { ${componentName} } from '${correction.to}';`
            );
            modified = true;
          }
        });
