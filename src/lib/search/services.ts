...
export class EnhancedJobSearchService {
  ...
  static async searchJobsEnhanced(
    ...
    // Try cache first
    const cached = await getCache<PaginatedResponse<SearchResult<any>>>(cacheKey);
    if (cached) {
    ...
    const whereCondition = await this.buildEnhancedWhereCondition(query, filters);
    ...
    if ('cursor' in pagination) {
      ...
      let cursorCondition = {};
      if (cursor) {
        cursorCondition = buildCursorCondition(cursor, 'createdAt', 'forward', 'desc');
      }
      ...
      const prevCursor = cursor && jobs.length > 0
    ...
    const facets = (filters as any).includeFacets ? 
      FacetedSearch.generateFacets(jobs, 'jobs') : undefined;
  ...
  private static async buildEnhancedWhereCondition(
    ...
    if (filters.location && !filters.lat && !filters.lng) {
      ...
      if (coords) {
        ...
        filters.radius = filters.radius || SEARCH_CONFIG.geolocation.defaultRadius;
      } else {
    ...
    if (filters.datePosted && typeof filters.datePosted === 'string') {
      try {
        ...
        if (isNaN(now.getTime())) {
          ...
          if (dateThreshold && !isNaN(dateThreshold.getTime()) && isFinite(dateThreshold.getTime())) {
            conditions.push({ createdAt: { gte: dateThreshold } });
          } else {
            console.warn('Invalid date threshold calculated for filter:', filters.datePosted);
          }
        }
      } catch (error) {
  ...
  private static scoreAndFilterResults(
    ...
    const results: SearchResult<any>[] = jobs.map(job => {
      ...
      const snippet = filters.includeSnippets && job.description
      ...
      async () => {
        ...
        if ('cursor' in pagination) {
          ...
          const paginationMeta = {
            ...
            nextCursor: hasNextPage ? generateCursorFromRecord(users[users.length - 1], 'updatedAt') : undefined,
          };
          ...
          [users, totalCount] = await Promise.all([
            prisma.user.findMany({
              where: whereCondition,
              orderBy: { updatedAt: 'desc' },
              skip: ((pagination as OffsetPaginationParams).page - 1) * pagination.limit,
          ...
          const { meta } = calculateOffsetPagination((pagination as OffsetPaginationParams).page, pagination.limit, totalCount);
