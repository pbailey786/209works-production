    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.trim().startsWith('} from ') && i > 0) {
        ...
        if (j >= 0 && !lines[j].includes('import {')) {
          ...
          if (importLine === 'import' || importLine.endsWith('import')) {
            ...
            modified = true;
            console.log(
              `Fixed missing import { in ${filePath} at line ${j + 1}`
            );
          }
        }
      }
      ...
      if (
        ...
        if (prevLine && prevLine.trim() === '') {
          ...
          modified = true;
          console.log(
            `Added missing import { in ${filePath} before line ${i + 1}`
          );
        }
      }
...
function fixSpecificFiles() {
  ...
  try {
    ...
    if (fs.existsSync(dropdownPath)) {
      ...
      if (
        content.includes("} from '@/hooks/useKeyboardNavigation';") &&
        !content.includes('import {')
      ) {
        content = content.replace(
          /^([^}]*)} from '@\/hooks\/useKeyboardNavigation';/m,
  ...
  try {
    ...
    if (fs.existsSync(jobCardPath)) {
      ...
      // Fix missing import {
      if (
        content.includes("} from '@/lib/types/component-props';") &&
        !content.includes('import {')
      ) {
        content = content.replace(
