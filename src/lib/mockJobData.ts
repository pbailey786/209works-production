...
export function generateEnhancedJobData(baseJob: any): EnhancedJobData {
  const companyNames = ['TechCorp', 'InnovateLabs', 'DataFlow Inc', 'CloudSolutions', 'DevSpace'];
  ...
  const skillSets: Record<string, Array<{ name: string; importance: 'required' | 'preferred' | 'nice-to-have' }>> = {
    'Software Engineer': [
  ...
  const jobTitleKey = Object.keys(skillSets).find(key => 
  ...
  return {
    ...
    benefits: benefits.sort(() => 0.5 - Math.random()).slice(0, 5 + Math.floor(Math.random() * 3)),
    ...
    employeeTestimonials: testimonials.sort(() => 0.5 - Math.random()).slice(0, 2),
    isRemote: Math.random() > 0.5,
    experienceLevel: ['Entry Level', 'Mid Level', 'Senior Level', 'Executive'][Math.floor(Math.random() * 4)]
  };
} 