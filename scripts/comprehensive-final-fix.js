  ...
  try {
    ...
    if (filePath.includes('/actions/')) {
      if (
        ...
        !content.includes('const { userId } = await auth()')
      ) {
    ...
    content = content.replace(
      /const\s*{\s*data:\s*session\s*}\s*=\s*useSession\(\);/g,
      'const { user } = useUser();'
    );
    ...
    content = content.replace(
      /user\?\.\s*emailAddresses\?\.\[0\]\?\.\s*emailAddress/g,
      'user?.email'
    );
    content = content.replace(/user\?\.\s*fullName/g, 'user?.name');
    content = content.replace(
      /user\?\.\s*publicMetadata\?\.\s*role/g,
      'user?.role'
    );
