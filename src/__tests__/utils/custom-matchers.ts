...
expect.extend({
  ...
  toBeValidJobType(received: string) {
    ...
    if (pass) {
      ...
      return {
        message: () =>
          `expected ${received} to be a valid job type. Valid types: ${validJobTypes.join(', ')}`,
        pass: false,
      };
    }
  },
  ...
  toHaveValidSalaryRange(received: { salaryMin?: number; salaryMax?: number }) {
    ...
    if (salaryMin !== undefined && salaryMax !== undefined) {
      ...
      if (pass) {
        return {
          message: () =>
            `expected salary range ${salaryMin}-${salaryMax} to be invalid`,
          pass: true,
        };
      } else {
        return {
          message: () =>
            `expected salary range to be valid (min: ${salaryMin}, max: ${salaryMax})`,
          pass: false,
        };
      }
    }
    ...
    return {
      message: () =>
        `expected object to have both salaryMin and salaryMax properties`,
      pass: false,
    };
  },
  ...
  toBeWithinDateRange(received: string | Date, startDate: Date, endDate: Date) {
    ...
    if (pass) {
      return {
        message: () =>
          `expected ${date.toISOString()} not to be within range ${startDate.toISOString()} - ${endDate.toISOString()}`,
        pass: true,
      };
    } else {
      return {
        message: () =>
          `expected ${date.toISOString()} to be within range ${startDate.toISOString()} - ${endDate.toISOString()}`,
        pass: false,
      };
    }
  },
