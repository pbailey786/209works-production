...
const SecurityConfig = z.object({
  ...
  CSRF_PROTECTION_ENABLED: z.string().default('true').transform(val => val === 'true'),
  RATE_LIMIT_ENABLED: z.string().default('true').transform(val => val === 'true'),
  SESSION_TIMEOUT_MINUTES: z.string().default('30').transform(val => parseInt(val, 10)),
  ...
  REQUIRE_2FA_FOR_ADMIN: z.string().default('true').transform(val => val === 'true'),
...
class CSRFProtection {
  ...
  static validateToken(token: string, sessionToken: string): boolean {
    ...
    const providedToken = createHash('sha256')
      .update(token)
      .digest('hex');
...
class AuthenticationValidator {
  static async validateUser(req: NextRequest): Promise<SecureAuthContext['user']> {
    ...
    return {
      ...
      lastActivity: new Date(),
      requiresMFA: user.role === 'admin' && SecurityConfig.REQUIRE_2FA_FOR_ADMIN,
      mfaVerified: !user.twoFactorEnabled || (session.user as any).mfaVerified === true,
      ipAddress,
      userAgent,
    };
  }
  ...
  static async checkPermissions(user: SecureAuthContext['user'], requiredPermissions: Permission[]): Promise<boolean> {
  ...
  static async checkRoles(user: SecureAuthContext['user'], requiredRoles: string[]): Promise<boolean> {
...
class EnhancedRateLimit {
  static async checkRateLimit(req: NextRequest, config: EnhancedSecurityConfig['rateLimit']): Promise<{
    allowed: boolean;
    remaining: number;
    resetTime: Date;
  }> {
...
class SecurityHeaders {
  static apply(response: NextResponse, config: EnhancedSecurityConfig['securityHeaders']): void {
...
export function withEnhancedSecurity(
  handler: (req: NextRequest, context: SecureAuthContext) => Promise<NextResponse>,
  ...
  return async (req: NextRequest): Promise<NextResponse> => {
    ...
    try {
      ...
      if (config.requireSecureConnection && !isSecureConnection && process.env.NODE_ENV === 'production') {
        SecurityLogger.suspiciousRequest(
          ipAddress,
      ...
      if (config.rateLimit?.enabled) {
        const rateLimitResult = await EnhancedRateLimit.checkRateLimit(req, config.rateLimit);
      ...
      // 6. CSRF protection
      if (config.csrfProtection?.enabled && ['POST', 'PUT', 'PATCH', 'DELETE'].includes(req.method)) {
      ...
      response.headers.set('X-RateLimit-Remaining', securityContext.security.rateLimitRemaining.toString());
