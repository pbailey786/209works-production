...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `[CRON] Found ${weeklyDigests.length} weekly digests to process`
    );
    ...
    for (const digest of weeklyDigests) {
      try {
        ...
        if (isUnsubscribed) {
          results.skipped++;
          console.log(
            `[CRON] Skipping digest ${digest.id} - user unsubscribed`
          );
          continue;
        }
        ...
        results.sent++;
        console.log(
          `[CRON] Successfully processed digest ${digest.id} with ${jobs.length} jobs`
        );
      } catch (error) {
        results.failed++;
    ...
    console.log(
      `[CRON] Weekly digest processing completed in ${processingTime}ms`,
      results
    );
...
async function getWeeklyDigestsToProcess(
  limit: number,
