...
export default function SEOHead({
  ...
  try {
    ...
    if (hostname) {
      ...
      domainConfig = {
        seo: {
          title:
            '209 Jobs - Your Local Job Board for Central Valley | No Remote Jobs from Utah',
          description:
            'Find meaningful work close to home in Stockton, Modesto, Tracy, Manteca, Lodi, and surrounding 209 communities. 100% local focus - no remote jobs from Utah or San Francisco.',
          keywords: [
            '209 jobs',
            'Central Valley jobs',
            'Stockton jobs',
  ...
  const finalOgImage =
    ogImage ||
    `https://${domainConfig.domain}/og-images/${domainConfig.domain.split('.')[0]}-og.jpg`;
  const fullTitle = finalTitle.includes(domainConfig.displayName)
    ? finalTitle
    : `${finalTitle} | ${domainConfig.displayName}`;
  ...
  return (
    <Head>
      ...
      <meta
        ...
        content={domainConfig.social.twitter || '@209jobs'}
      />
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            name: '209 Jobs',
            description:
              'Local job board serving the Central Valley of California',
            url: canonical,
