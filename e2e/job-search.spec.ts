...
test.describe('Job Search', () => {
  test('should perform a basic job search', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      if (await searchButton.isVisible()) {
        ...
        const resultsIndicator = page.locator('.job-results, [data-testid="job-results"], .search-results').first()
  ...
  test('should display job listings', async ({ page }) => {
    ...
    const jobListings = page.locator('.job-card, .job-listing, [data-testid="job-card"]')
  ...
  test('should open job details when clicking on a job', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const jobDetails = page.locator('.job-details, [data-testid="job-details"], .modal')
  ...
  test('should handle empty search results gracefully', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      if (await searchButton.isVisible()) {
        ...
        const noResults = page.locator(':has-text("No jobs found"), :has-text("No results"), .no-results')
