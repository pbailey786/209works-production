...
export const LOCAL_AREAS: LocalArea[] = [
  {
    ...
    county: "San Joaquin",
    majorEmployers: ["San Joaquin Delta College", "St. Joseph's Medical Center", "Amazon", "Port of Stockton"],
    industries: ["logistics", "healthcare", "education", "agriculture", "distribution"],
    landmarks: ["Port of Stockton", "Delta College", "Miracle Mile", "Lincoln Center"],
    commuteInfo: {
  ...
  },
  {
    ...
    county: "Stanislaus",
    majorEmployers: ["Gallo Winery", "Memorial Medical Center", "Modesto City Schools", "Foster Farms"],
    industries: ["food processing", "agriculture", "healthcare", "education", "manufacturing"],
  ...
  },
  {
    ...
    county: "San Joaquin",
    majorEmployers: ["Lodi Unified School District", "Adventist Health Lodi Memorial", "General Mills"],
    industries: ["wine", "agriculture", "education", "healthcare", "food processing"],
...
export const LOCAL_INDUSTRIES = {
  "agriculture": {
    ...
    commonJobs: ["farm worker", "agricultural technician", "crop advisor", "farm manager"],
    seasonality: "Seasonal hiring peaks: Spring (planting), Summer/Fall (harvest)"
  },
  "logistics": {
    description: "Major distribution hub for California",
    commonJobs: ["warehouse worker", "forklift operator", "logistics coordinator", "truck driver"],
    majorHubs: ["Amazon", "Costco", "FedEx", "UPS"]
  },
  "healthcare": {
    description: "Growing healthcare sector serving Central Valley",
    commonJobs: ["nurse", "medical assistant", "healthcare administrator", "technician"],
...
export const COMMUTE_PATTERNS = {
  "reverse_commute": {
    ...
    benefits: ["Lower housing costs", "Better quality of life", "Family-friendly communities"]
  },
  "local_opportunities": {
    description: "Growing local job market reduces commute need",
    trends: ["Tech companies expanding to Central Valley", "Healthcare growth", "Logistics boom"]
  }
};
...
export class LocalKnowledgeService {
  ...
  static getLocalJobContext(industry?: string, location?: string): string {
    ...
    if (industry && LOCAL_INDUSTRIES[industry as keyof typeof LOCAL_INDUSTRIES]) {
      const industryInfo = LOCAL_INDUSTRIES[industry as keyof typeof LOCAL_INDUSTRIES];
  ...
  static getSalaryContext(role?: string, industry?: string): string {
    ...
    if (industry && LOCAL_SALARY_CONTEXT.industry_salaries[industry as keyof typeof LOCAL_SALARY_CONTEXT.industry_salaries]) {
      const industryData = LOCAL_SALARY_CONTEXT.industry_salaries[industry as keyof typeof LOCAL_SALARY_CONTEXT.industry_salaries];
  ...
  static getCommuteVsLocalAdvice(userSalaryRange?: string): string {
    ...
    advice.push("\n💡 Recommendation:");
    advice.push("Consider local opportunities first - the quality of life benefits often outweigh salary differences. Many residents find local careers more fulfilling and sustainable long-term.");
