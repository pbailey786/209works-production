...
export function formatDate(date: Date | string | number): string {
  ...
  try {
    ...
    if (date instanceof Date) {
      ...
      if (!UtilsValidator.isValidNumber(date) || date < 0 || date > Date.now() + 365 * 24 * 60 * 60 * 1000) {
    ...
    if (baseUrl) {
      try {
        ...
        if (!['http:', 'https:'].includes(url.protocol)) {
          console.error('Invalid protocol in NEXT_PUBLIC_APP_URL:', url.protocol);
...
export function truncate(str: string, length: number): string {
  ...
  try {
    ...
    console.error('Error truncating string:', error, 'String:', str, 'Length:', length);
...
export async function getGitHubStars(): Promise<number | null> {
  try {
    ...
    const repoUrl = "https://api.github.com/repos/pjborowiecki/SAASY-LAND-Next-14-Starters-With-Authentication-And-Database-Implemented";
    ...
    try {
      ...
      if (!UtilsValidator.isValidNumber(starCount) || starCount < 0) {
        console.error('GitHub API returned invalid stargazers_count:', starCount);
        return null;
      }
      ...
      if (fetchError instanceof Error) {
        if (fetchError.name === 'AbortError') {
          ...
          console.error('Network error accessing GitHub API:', fetchError.message);
        } else {
          console.error('Fetch error accessing GitHub API:', fetchError.message);
        }
      } else {
...
export function addDays(date: Date | string | number, days: number): Date | null {
  if (!UtilsValidator.isValidDate(date) || !UtilsValidator.isValidNumber(days)) {
...
export function getTimeDifferenceInHours(date1: Date | string | number, date2: Date | string | number): number | null {
  if (!UtilsValidator.isValidDate(date1) || !UtilsValidator.isValidDate(date2)) {
    console.warn('getTimeDifferenceInHours received invalid dates:', { date1, date2 });
    return null;
  }
...
export function formatRelativeDate(date: Date | string | number): string {
  ...
  try {
    ...
    if (!UtilsValidator.isValidDate(inputDate) || !UtilsValidator.isValidDate(now)) {
      return 'Invalid Date';
    }
