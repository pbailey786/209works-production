...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  if (event.httpMethod !== 'POST' && event.headers['netlify-function-cron'] !== 'true') {
    return {
      statusCode: 401,
      body: JSON.stringify({ error: 'Unauthorized - not a scheduled function' })
    };
  }

  try {
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || process.env.URL || 'https://209works.com';
